...
設定項目として使える文字列など、書き方の詳細は
hotkey.ini を参照してください。
...
A:「日本語入力でも直接入力でも入力文字数が変わらない」ような短縮形を使います。

  例: 短縮形'asu@', 定型文'また明日' の場合
  定型文挿入時、本ソフトウェアは4文字分('asu@'の4文字)の文字を消去します。
  直接入力モードだとこれが上手く働きますが、日本語入力中だとそうはいきません。
  日本語入力だと 'あす@' となっており3文字であるため、余分に1文字分消去されます。

  これを防ぐために、たとえば短縮形を 'as@' にします。
